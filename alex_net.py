...
from layers import DataLayer, ConvPoolLayer, DropoutLayer, FCLayer, SoftmaxLayer,
                   LogisticRegressionLayer, Deconvolutional_Layer, GaussianLayer

class Variational_AlexNet(object):
    """ This implements a variational version of alexnet """
    def __init__(self, config):
        ...
        # Dropout Layer no parameters just random drop out
        dropout_layer6 = DropoutLayer(fc_layer6.output, n_in=4096, n_out=4096)

        # Now we need to get the output or what we want z to be
        # Now here is the Guassian Probability Section with the gaussian layer
        gauss_layer = GaussianLayer(input=dropout_layer6.output, n_in=4096, n_out=4096)
        self.layers.append(gauss_layer)
        params += gauss_layer.params
        weight_types += fc_layer6.weight_type

        ############ Sampling Layer ##########################
        #Create the sampled output layer
        z = gauss_layer.mu_encoded + T.exp(gauss_layer.log_sig_encoded) * eps
        prior_z = 0.5* T.sum(1 + 2*gauss_layer.log_sig_encoded - gauss_layer.mu_encoded**2 - T.exp(2*gauss_layer.log_sig_encoded)) 

        ############ GENERATIVE MODEL ########################
        fc_layer_d1 = FClayer(input=z, n_in=4096, n_out=9216)
        self.layers.append(fc_layer_d1)
        params += fc_layer_d1.params
        weight_types += fc_layer_d1.weight_type

        # De_Convolutional Layer 1
        deconvpool_layer1 = Deconvolutional_Layer(input=fc_layer_d1.output,
                                        channel=1,
                                        filter_shape=(384, 3, 3, 256), 
